...
if __name__ == '__main__':
    ...
    videoOut = cv2.VideoWriter(outFileName, cv2.VideoWriter_fourcc('m', 'p', 'e', 'g'), int(videoIn.get(cv2.CAP_PROP_FRAME_COUNT)), (int(videoIn.get(cv2.CAP_PROP_FRAME_WIDTH)), int(videoIn.get(cv2.CAP_PROP_FRAME_HEIGHT))))
    ...
    while(videoIn.grab()):
        ...
        binary = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 31, 40) ## + cv2.THRESH_OTSU)
